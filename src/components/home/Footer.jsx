...
function NewsletterForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitted(true); // Show success to avoid user frustration on duplicates
      setTimeout(() => setSubmitted(false), 3000);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="relative space-y-3">
      <div className="relative">
        ...
        <Input
          ...
          required
          className={`${isRTL ? 'pr-10 pl-3' : 'pl-10 pr-3'} py-5 bg-white/10 border-white/20 text-white placeholder:text-white/60 rounded-xl focus:bg-white/20 focus:border-[#42C0B9]`}
        />
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          <div className="flex flex-col items-start">
            ...
            <div className="flex gap-3">
              {[Twitter, Linkedin, Facebook, Instagram].map((Icon, index) => (
                <a key={index} href="#" className="p-2 rounded-lg bg-white/10 text-white/60 hover:text-[#42C0B9] hover:bg-white/20 transition-colors">
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li>
                <Link to="/terms" className="text-white/60 hover:text-[#42C0B9] transition-colors inline-block">
                  {isRTL ? 'תנאי שימוש' : 'Terms of Service'}
                </Link>
              </li>
              <li>
                <Link to="/privacy" className="text-white/60 hover:text-[#42C0B9] transition-colors inline-block">
                  {isRTL ? 'מדיניות פרטיות' : 'Privacy Policy'}
                </Link>
              </li>
              <li>
                <Link to="/cookies" className="text-white/60 hover:text-[#42C0B9] transition-colors inline-block">
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6 order-1 sm:order-2">
             <button onClick={scrollToTop} className="p-2 rounded-full bg-white/5 border border-white/10 text-white/60 hover:text-[#42C0B9] transition-all">
